...
class UIManager {
  ...
  restorePreviewInterface() {
      ...
      this.attachPreviewEventListeners(this.sidebarElement);
    } else {
  ...
  attachPreviewEventListeners(sidebarElement) {
    const self = this;
    ...
    const previewOutput = sidebarElement.querySelector('#preview-output');
    const refreshBtn = sidebarElement.querySelector('#preview-refresh-btn');
    const closeBtn = sidebarElement.querySelector('#preview-close-btn');

    // Refresh preview
    refreshBtn.addEventListener('click', () => {
      console.log('[PREVIEW] Refresh button clicked');
      // Clear current preview and show default state
      previewOutput.innerHTML = `
        ...
        ">
          ...
          <p style="
            ...
            color: #64748b;
            line-height: 1.5;
          ">Click on any document icon in the spreadsheet to preview it here. This panel will show document details, thumbnails, and quick actions.</p>
        </div>
      `;
    });
    ...
    closeBtn.addEventListener('click', () => {
      ...
      if (self.sidebarPanel) {
        self.sidebarPanel.close();
      }
    });

    // Store reference to preview output for external access
    self.previewOutput = previewOutput;
  }

  /**
   * Shows document preview in the sidebar panel
   * @param {Object} documentData - Document data including URL, name, type, etc.
   */
  showDocumentPreview(documentData) {
    if (!this.previewOutput) {
      console.warn('[PREVIEW] Preview output not available');
      return;
    }

    const { docUrl, docName, thumbUrl, fileType } = documentData;
    const isImage = /\.(jpg|jpeg|png|gif|webp|svg|bmp|tiff)$/i.test(docUrl);
    const isPdf = /\.pdf$/i.test(docUrl);

    console.log('[PREVIEW] Showing document preview:', { docUrl, docName, fileType });
    ...
    this.previewOutput.innerHTML = `
      ...
      ">
        ...
        ">
          ${docUrl ? `
            <div style="flex: 1; min-height: 300px; position: relative; background: #ffffff; border-radius: 8px 8px 0 0; overflow: hidden;">
              ${isImage ? `
                <!-- Image Viewer -->
                <div style="width: 100%; height: 100%; position: relative; background: #f8fafc; display: flex; align-items: center; justify-content: center;">
                  <img src="${docUrl}" 
                  ...
                  <div style="display: none; text-align: center; color: #64748b; padding: 20px;">
          ...
          ` : `
            <div style="flex: 1; display: flex; align-items: center; justify-content: center; background: #f8fafc; min-height: 200px;">
          ...
          <!-- Document Info -->
          <div style="background: #ffffff; padding: 16px; border-top: 1px solid #e2e8f0;">
            <div style="font-size: 14px; font-weight: 600; margin-bottom: 8px; color: #1e293b;">Document Details</div>
            <div style="font-size: 12px; color: #64748b; margin-bottom: 4px;"><strong>File:</strong> ${docName}</div>
            <div style="font-size: 12px; color: #64748b; margin-bottom: 4px;"><strong>Type:</strong> ${fileType || (isImage ? 'Image' : (isPdf ? 'PDF Document' : 'Document'))}</div>
            <div style="font-size: 12px; color: #64748b; margin-bottom: 8px;"><strong>Status:</strong> <span style="color: #10b981;">Ready</span></div>
                ...
                ">
                    ...
                    ">
                        <div style="
                            ...
                            flex-wrap: wrap;
                        ">
                            <button id="preview-refresh-btn" style="
                                padding: 12px 20px;
                                background: linear-gradient(135deg, #10b981 0%, #059669 100%);
                                color: white;
                                border: none;
  ...
  renderMainView() {
    ...
    const el = this.createAndMount(this.createMainAppContent(), async (el) => {
        ...
        this.attachPreviewEventListeners(el); // Call the new method here
        console.log('[UI DEBUG] Preview event listeners attached');
