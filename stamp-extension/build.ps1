...
if ($env:PREFER_GOOGLE_CLIENT -eq "true") {
    ...
    Write-Host "   Selected CLIENT_SECRET: $(if ($FINAL_CLIENT_SECRET) { $FINAL_CLIENT_SECRET.Substring(0, [Math]::Min(10, $FINAL_CLIENT_SECRET.Length)) } else { 'NOT_SET' })..." -ForegroundColor Yellow
...
Write-Host "   Chrome OAuth client ID: '$(if ($FINAL_CHROME_CLIENT_ID) { $FINAL_CHROME_CLIENT_ID } else { 'NOT_SET' })'" -ForegroundColor Yellow
...
$contentBuildArgs = @(
    ...
    "--sourcemap",
    "--minify=false",
    "--define:CONFIG.API_ENDPOINT='$env:API_ENDPOINT'",
    "--define:CONFIG.AUTH_ENDPOINT='$env:AUTH_ENDPOINT'",
    "--define:CONFIG.CLIENT_ID='$(if ($FINAL_CLIENT_ID) { $FINAL_CLIENT_ID } else { '' })'",
    "--define:CONFIG.CLIENT_SECRET='$(if ($FINAL_CLIENT_SECRET) { $FINAL_CLIENT_SECRET } else { '' })'",
    "--define:CONFIG.CHROME_CLIENT_ID='$(if ($FINAL_CHROME_CLIENT_ID) { $FINAL_CHROME_CLIENT_ID } else { '' })'"
)
npx esbuild @contentBuildArgs
...
npx esbuild @backgroundBuildArgs
...
$oauthBuildArgs = @(
    ...
    "--bundle",
    "--outfile=dist/oauth-callback-detector.js",
    "--platform=browser",
    "--sourcemap",
    "--minify=false"
)
npx esbuild @oauthBuildArgs
...
if (Test-Path "node_modules/@inboxsdk/core/pageWorld.js") {
    ...
    Write-Host "   Warning: pageWorld.js not found - skipping InboxSDK files" -ForegroundColor Yellow
}

if (Test-Path "node_modules/@inboxsdk/core/pageWorld.js.map") {
    Copy-Item "node_modules/@inboxsdk/core/pageWorld.js.map" "dist/"
    Write-Host "   pageWorld.js.map copied successfully" -ForegroundColor Yellow
} else {
    Write-Host "   Warning: pageWorld.js.map not found - skipping" -ForegroundColor Yellow
}
